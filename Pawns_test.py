...
class PawnsTest(unittest.TestCase):
    ...
    def test_can_capture(self):
        ...
        self.assertTrue(Pa.can_capture(1, 1, self.board, remembered_x, remembered_y, self.player_turn))

    def test_can_move1(self):
        self.board[0][1].player = Pa.NULL
        remembered_x, remembered_y = 0, 2
        self.assertFalse(Pa.can_move1(self.board, remembered_x, remembered_y, self.player_turn))

    def test_can_move2(self):
        self.board[0][1].player = Pa.NULL
        Pa.remembered_x,remembered_y = 0, 2
        self.assertFalse(Pa.can_move2(0, 1, self.board, self.remembered_x, remembered_y))

    def test_can_select(self):
        remembered_x, remembered_y = 0, 3
        self.assertTrue(Pa.can_select(self.board, remembered_x, remembered_y, self.player_turn))

    def test_capture(self):
        remembered_x, remembered_y = 1, 3
        Pa.capture_or_move(self.board, self.do_reset, remembered_x, remembered_y, self.player_turn, (1, 1))
        self.assertEqual(Pa.WHITE, self.board[1][1].player)

    def test_move(self):
        remembered_x, remembered_y = 1, 1
        self.board[1][0].player = Pa.NULL
        Pa.capture_or_move(self.board, self.do_reset, remembered_x, remembered_y, self.player_turn, (1, 0))
